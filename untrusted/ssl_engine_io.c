...
{
    bio_filter_out_ctx_t *outctx; sgx_BIO_get_data(global_eid, (void**)&outctx, bio);// = (bio_filter_out_ctx_t *)BIO_get_data(bio);
    apr_bucket *e;
...
{
	sgx_BIO_set_shutdown(global_eid, bio, 1);
	sgx_BIO_set_init(global_eid, bio, 1);
    ...
    bio->num = -1;
#endif
	sgx_BIO_set_data(global_eid, bio, NULL);
    return 1;
}
...
static int bio_filter_out_write(BIO bio, const char *in, int inl)
{
	...
	sgx_BIO_clear_retry_flags(global_eid, bio);
...
#else
	sgx_SSL_in_connect_init(global_eid, &need_flush, outctx->filter_ctx->pssl);
...
static long bio_filter_out_ctrl(BIO bio, int cmd, long num, void *ptr)
{
    ...
    case BIO_CTRL_GET_CLOSE:
		sgx_BIO_get_shutdown(global_eid, &ret, bio);
        break;
      case BIO_CTRL_SET_CLOSE:
		sgx_BIO_set_shutdown(global_eid, bio, num);
        break;
      case BIO_CTRL_FLUSH:
...
static int bio_filter_in_read(BIO bio, char *in, int inlen)
{
    ...
    bio_filter_in_ctx_t *inctx; sgx_BIO_get_data(global_eid, (void**)&inctx, bio);
    ...
    if (APR_BRIGADE_EMPTY(inctx->bb)) {
        ...
        if (APR_STATUS_IS_EAGAIN(inctx->rc) || APR_STATUS_IS_EINTR(inctx->rc)
            ...
            sgx_BIO_set_retry_read(global_eid, bio);
            return -1;
        }
...
static WOLFSSL_BIO_METHOD_IDENTIFIER bio_filter_out_method = INVALID_IDENTIFIER;
static WOLFSSL_BIO_METHOD_IDENTIFIER bio_filter_in_method = INVALID_IDENTIFIER;

void init_bio_methods(void)
{
    sgx_SSL_BIO_meth_new(global_eid, &bio_filter_out_method, BIO_TYPE_MEM, "APR output filter");

    sgx_BIO_meth_set_write(global_eid, NULL, bio_filter_out_method, &bio_filter_out_write);
    sgx_BIO_meth_set_read(global_eid, NULL, bio_filter_out_method, &bio_filter_out_read); /* read is never called */
    sgx_BIO_meth_set_puts(global_eid, NULL, bio_filter_out_method, &bio_filter_out_puts); /* puts is never called */
    sgx_BIO_meth_set_gets(global_eid, NULL, bio_filter_out_method, &bio_filter_out_gets); /* gets is never called */
    sgx_BIO_meth_set_ctrl(global_eid, NULL, bio_filter_out_method, &bio_filter_out_ctrl);
    sgx_BIO_meth_set_create(global_eid, NULL, bio_filter_out_method, &bio_filter_create);
    sgx_BIO_meth_set_destroy(global_eid, NULL, bio_filter_out_method, &bio_filter_destroy);

    sgx_SSL_BIO_meth_new(global_eid, &bio_filter_in_method, BIO_TYPE_MEM, "APR input filter");
    sgx_BIO_meth_set_write(global_eid, NULL, bio_filter_in_method, &bio_filter_in_write); /* write is never called */
    sgx_BIO_meth_set_read(global_eid, NULL, bio_filter_in_method, &bio_filter_in_read);
    sgx_BIO_meth_set_puts(global_eid, NULL, bio_filter_in_method, &bio_filter_in_puts);   /* puts is never called */
    sgx_BIO_meth_set_gets(global_eid, NULL, bio_filter_in_method, &bio_filter_in_gets);   /* gets is never called */
    sgx_BIO_meth_set_ctrl(global_eid, NULL, bio_filter_in_method, &bio_filter_in_ctrl);   /* ctrl is never called */
    sgx_BIO_meth_set_create(global_eid, NULL, bio_filter_in_method, &bio_filter_create);
    sgx_BIO_meth_set_destroy(global_eid, NULL, bio_filter_in_method, &bio_filter_destroy);
}

void free_bio_methods(void)
{
    sgx_BIO_meth_free(global_eid, bio_filter_out_method);
    sgx_BIO_meth_free(global_eid, bio_filter_in_method);
}
#endif
                                      ...
                                      apr_size_t *len)
{
    ...
    *len = 0;
    ...
    while (1) {
         ...
         */
        sgx_ERR_clear_error(global_eid);
        ...
        sgx_SSL_read(global_eid, &rc, inctx->filter_ctx->pssl, buf + bytes, wanted - bytes);
        ...
        if (rc > 0) {
            *len += rc;
            ...
            sgx_SSL_get_error(global_eid, &ssl_err, inctx->filter_ctx->pssl, rc);
			sgx_SSL_get_app_data(global_eid, (void**)&c, inctx->filter_ctx->pssl);
            ...
            else if (ssl_err == WOLFSSL_ERROR_SYSCALL) {
                if (APR_STATUS_IS_EAGAIN(inctx->rc)
                        || APR_STATUS_IS_EINTR(inctx->rc)) {
                    /* Already read something, return APR_SUCCESS instead. */
                    if (*len > 0) {
            ...
            else if (rc == 0 && ssl_err == WOLFSSL_ERROR_ZERO_RETURN) {
                inctx->rc = APR_EOF;
                break;
            }
                                     ...
                                     apr_size_t len)
{
    ...
    /* write SSL */
    if (filter_ctx->pssl == INVALID_IDENTIFIER) {
        return APR_EGENERAL;
    }
	...
	sgx_BIO_get_data(global_eid, (void**)&outctx, filter_ctx->pbioWrite);
	sgx_SSL_write(global_eid, &res, filter_ctx->pssl, (unsigned char*)data, len);

    if (res < 0) {
		int ssl_err;
        sgx_SSL_get_error(global_eid, &ssl_err,filter_ctx->pssl, res);
		conn_rec *c;
        sgx_SSL_get_app_data(global_eid, (void**) &c, outctx->filter_ctx->pssl);

        if (ssl_err == WOLFSSL_ERROR_WANT_WRITE) {
            /*
        ...
        else if (ssl_err == WOLFSSL_ERROR_WANT_READ) {
            /*
		...
		long result; 
        if (sgx_SSL_total_renegotiations(global_eid, &result, filter_ctx->pssl) == SGX_SUCCESS && result) {
...
{
    sgx_SSL_free(global_eid, inctx->ssl);
    sslconn->ssl = INVALID_IDENTIFIER;
    inctx->ssl = INVALID_IDENTIFIER;
    inctx->filter_ctx->pssl = INVALID_IDENTIFIER;
}
                                   ...
                                   conn_rec *c, int abortive)
{
     ...
     */
    if (abortive) {
        shutdown_type = WOLFSSL_SENT_SHUTDOWN|WOLFSSL_RECEIVED_SHUTDOWN;
        type = "abortive";
        logno = APLOGNO(01998);
        loglevel = APLOG_INFO;
    }
      ...
      default:
        ...
        shutdown_type = WOLFSSL_RECEIVED_SHUTDOWN;
        type = "standard";
        logno = APLOGNO(02001);
        break;
    }

    sgx_SSL_set_shutdown(global_eid, ssl, shutdown_type);
    ...
    if (sslconn->client_cert) {
        sgx_X509_free(global_eid, sslconn->client_cert);
        sslconn->client_cert = INVALID_IDENTIFIER;
    }
    sgx_SSL_free(global_eid, ssl);
    sslconn->ssl = INVALID_IDENTIFIER;
    filter_ctx->pssl = INVALID_IDENTIFIER; /* so filters know we've been shutdown */
...
static apr_status_t ssl_io_filter_cleanup(void *data)
{
    ...
    if (filter_ctx->pssl) {
        conn_rec *c; sgx_SSL_get_app_data(global_eid, (void**)&c, filter_ctx->pssl);
        SSLConnRec *sslconn = myConnConfig(c);

        sgx_SSL_free(global_eid, filter_ctx->pssl);
        sslconn->ssl = filter_ctx->pssl = INVALID_IDENTIFIER;
    }
...
static apr_status_t ssl_io_filter_handshake(ssl_filter_ctx_t *filter_ctx)
{
  ...
  //  conn_rec *c         = (conn_rec *)SSL_get_app_data(filter_ctx->pssl);
    ...
    if (sgx_SSL_is_init_finished(global_eid, &is_init_finished, filter_ctx->pssl) == SGX_SUCCESS && is_init_finished) {
        return APR_SUCCESS;
    }
    ...
    if (sslconn->is_proxy) {
        ...
        if (hostname_note &&
            ...
            apr_ipsubnet_create(&ip, hostname_note, NULL,
                                c->pool) != APR_SUCCESS) {
								int _setHost;
            if (sgx_SSL_set_tlsext_host_name(global_eid, &_setHost, filter_ctx->pssl, hostname_note) == SGX_SUCCESS && _setHost) {
        ...
        if (sgx_SSL_connect(global_eid, &n, filter_ctx->pssl) == SGX_SUCCESS && n <= 0) {
        ...
        sgx_SSL_get_peer_certificate(global_eid, &cert, filter_ctx->pssl);

        if (dc->proxy->ssl_check_peer_expire != FALSE) {

			ASN1_TIME notBefore; sgx_X509_get_notBefore(global_eid, &notBefore, cert);
			ASN1_TIME notAfter; sgx_X509_get_notAfter(global_eid, &notAfter, cert);
			int notBeforeComp; sgx_X509_cmp_current_time(global_eid, &notBeforeComp, notBefore);
			int notAfterComp; sgx_X509_cmp_current_time(global_eid, &notAfterComp, notAfter);

            if (!cert || notBeforeComp >= 0 || notAfterComp <= 0) {
        ...
        if (proxy_ssl_check_peer_ok == TRUE) {
            ...
            post_handshake_rc = ssl_run_proxy_post_handshake(c, (SSL*)filter_ctx->pssl);
        }

        if (cert) {
            sgx_X509_free(global_eid, cert);
        }
     ...
     */
    sgx_ERR_clear_error(global_eid);
    if (sgx_SSL_accept(global_eid, &n, filter_ctx->pssl) == SGX_SUCCESS &&  n <= 0) {
        bio_filter_in_ctx_t *inctx; sgx_BIO_get_data(global_eid, (void**)&inctx, 
										filter_ctx->pbioRead);
        bio_filter_out_ctx_t *outctx;  sgx_BIO_get_data(global_eid, (void**)&inctx, 
										filter_ctx->pbioWrite);
        apr_status_t rc = inctx->rc ? inctx->rc : outctx->rc ;
        sgx_SSL_get_error(global_eid, &ssl_err, filter_ctx->pssl, n);
		int errorPeek;
        if (ssl_err == WOLFSSL_ERROR_ZERO_RETURN) {
            /*
     ...
     */
    sgx_SSL_get_verify_result(global_eid, &verify_result, filter_ctx->pssl);
     ...
     */
    if (sgx_SSL_get_peer_certificate(global_eid, &cert, filter_ctx->pssl) == SGX_SUCCESS && cert) {
        if (sslconn->client_cert) {
            sgx_X509_free(global_eid, sslconn->client_cert);
        }
                                         ...
                                         apr_bucket_brigade *bb)
{
    ...
    sgx_BIO_get_data(global_eid, (void**)&inctx, filter_ctx->pbioRead);//inctx = (bio_filter_in_ctx_t *)BIO_get_data(filter_ctx->pbioRead);
    sgx_BIO_get_data(global_eid, (void**)&outctx, filter_ctx->pbioWrite);//outctx = (bio_filter_out_ctx_t *)BIO_get_data(filter_ctx->pbioWrite);
    ...
    if (status == APR_SUCCESS) {
        status = ap_filter_setaside_brigade(f, bb); // TODO find core.h
    }
    return status;
}
...
#else
	sgx_BIO_new(global_eid, &filter_ctx->pbioRead, bio_filter_in_method);
#endif
	sgx_BIO_set_data(global_eid, filter_ctx->pbioRead, (void *)inctx);
...
void ssl_io_filter_init(conn_rec *c, request_rec *r, SSL ssl)
{
...
#else
	sgx_BIO_new(global_eid, &filter_ctx->pbioWrite, bio_filter_out_method);
#endif
    //BIO_set_data(filter_ctx->pbioWrite, (void *)bio_filter_out_ctx_new(filter_ctx, c));
	sgx_BIO_set_data(global_eid, filter_ctx->pbioWrite, bio_filter_out_ctx_new(filter_ctx, c));
    /* write is non blocking for the benefit of async mpm */
    if (c->cs) {
		sgx_BIO_set_nbio(global_eid, NULL, filter_ctx->pbioWrite, 1);
    ...
    sgx_SSL_set_bio(global_eid, ssl, filter_ctx->pbioRead, filter_ctx->pbioWrite);
    filter_ctx->pssl            = ssl;
    ...
    if (APLOG_CS_IS_LEVEL(c, mySrvFromConn(c), APLOG_TRACE4)) {
		BIO rbio, wbio;
		sgx_SSL_get_rbio(global_eid, &rbio, ssl);
		sgx_SSL_get_wbio(global_eid, &rbio, ssl);


        sgx_BIO_set_callback(global_eid, rbio, ssl_io_data_cb);
        sgx_BIO_set_callback_arg(global_eid, rbio, (void *)ssl);
        if (wbio && wbio != rbio) {
			sgx_BIO_set_callback(global_eid, wbio, ssl_io_data_cb);
			sgx_BIO_set_callback_arg(global_eid, wbio, (void *)ssl);
        }
    }
                    ...
                    int argi, long argl, long rc)
{
    ...
    if (sgx_BIO_get_callback_arg(global_eid, (void**)&ssl, bio) == SGX_SUCCESS && ssl == INVALID_IDENTIFIER)
        return rc;
    if (sgx_SSL_get_app_data(global_eid, (void**) &c, ssl) == SGX_SUCCESS && c == NULL)
        return rc;
    s = mySrvFromConn(c);

    if (   cmd == (WOLFSSL_BIO_CB_WRITE|WOLFSSL_BIO_CB_RETURN)
        || cmd == (WOLFSSL_BIO_CB_READ |WOLFSSL_BIO_CB_RETURN) ) {
        if (rc >= 0) {
            const char *dump = "";
            ...
            ap_log_cserror(APLOG_MARK, APLOG_TRACE4, 0, c, s,
                    ...
                    MODSSL_LIBRARY_NAME,
                    (cmd == (WOLFSSL_BIO_CB_WRITE|WOLFSSL_BIO_CB_RETURN) ? "write" : "read"),
                    rc, argi, (cmd == (WOLFSSL_BIO_CB_WRITE|WOLFSSL_BIO_CB_RETURN) ? "to" : "from"),
                    (void*)bio, argp, dump);
            if (*dump != '\0' && argp != NULL)
...
int 	do_BIO_meth_write_cb(	BIO bioId, const char *in, 	int inl, 	void* callback)
{
	int(*f)(BIO, const char*, int) = callback;
	return f(bioId, in, inl);
}
...
int 	do_BIO_meth_puts_cb(	BIO bioId, const char *in,			 	void* callback)
{
	int(*f)(BIO, const char*) = callback;
	return f(bioId, in);
}
int 	do_BIO_meth_gets_cb(	BIO bioId, char *out, 	 	int inl, 	void* callback)
{
	int(*f)(BIO, char*, int) = callback;
	return f(bioId, out, inl);
}
		//TODO pass ptr?
long 	do_BIO_meth_ctrl_cb(	BIO bioId, 					int cmd,		long num, 	void* callback)
{
		int(*f)(BIO, int, long, void*) = callback;
	return f(bioId, cmd, num, callback);
}
int 	do_BIO_meth_create_cb(	BIO bioId, void* callback)
{
	int(*f)(BIO) = callback;
	return f(bioId);
}
int 	do_BIO_meth_destroy_cb(	BIO bioId, void* callback)
{
	int(*f)(BIO) = callback;
	return f(bioId);
}

long do_BIO_info_cb(BIO bioId, int event, const char* parg, int iarg, long larg, long return_value, void* callback)
{
	long(*f)(BIO, int, const char*, int, long, long) = callback;
	return f(bioId, event, parg, iarg, larg, return_value);
}