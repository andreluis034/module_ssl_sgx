...
int ssl_rand_seed(server_rec *s, apr_pool_t *p, ssl_rsctx_t nCtx, char *prefix)
{
    ...
    for (i = 0; i < apRandSeed->nelts; i++) {
        ...
        if (pRandSeed->nCtx == nCtx) {
            if (pRandSeed->nSrc == SSL_RSSRC_FILE) {
                ...
                l = sizeof(my_seed);
				sgx_RAND_seed(global_eid,NULL, (char*) &my_seed, l);
                nDone += l;
				...
				sgx_RAND_seed(global_eid,NULL, stackdata+n, 128);
                nDone += 128;
	...
	int status = 0;
    if (sgx_RAND_status(global_eid, &status) == SGX_SUCCESS || status == 0)
...
static int ssl_rand_feedfp(apr_pool_t *p, apr_file_t *fp, int nReq)
{
    ...
    while (1) {
        ...
        if (apr_file_read(fp, caBuf, &nBuf) != APR_SUCCESS)
            break;
		sgx_RAND_seed(global_eid,NULL, caBuf, nBuf);
        nDone += nBuf;
        if (nReq > 0) {
            nTodo -= nBuf;
            if (nTodo <= 0)
