...
static char *ssl_var_lookup_ssl_cert_chain(apr_pool_t *p, SSL_STACK sk, char *var);
                                   ...
                                   unsigned char **buf, apr_size_t *size)
{
    ...
    if (l > 0) {
        ...
        if (md == INVALID_IDENTIFIER || md == EVP_md5() || md == EVP_sha1()) {
            md = EVP_sha256();
        }
                                ...
                                request_rec *r, char *var)
{
    ...
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "PROTOCOL")) {
        result = (char *)SSL_get_version(ssl);
    }
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "SESSION_ID")) {
    ...
    else if(ssl != INVALID_IDENTIFIER && strcEQ(var, "SESSION_RESUMED")) {
        if (SSL_session_reused(ssl) == 1)
            result = "Resumed";
        else
            result = "Initial";
    }
    else if (ssl != INVALID_IDENTIFIER && strlen(var) >= 6 && strcEQn(var, "CIPHER", 6)) {
        result = ssl_var_lookup_ssl_cipher(p, sslconn, var+6);
    }
    else if (ssl != INVALID_IDENTIFIER && strlen(var) > 18 && strcEQn(var, "CLIENT_CERT_CHAIN_", 18)) {
        sk = SSL_get_peer_cert_chain(ssl);
        result = ssl_var_lookup_ssl_cert_chain(p, sk, var+18);
    }
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "CLIENT_CERT_RFC4523_CEA")) {
        result = ssl_var_lookup_ssl_cert_rfc4523_cea(p, ssl);
    }
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "CLIENT_VERIFY")) {
        result = ssl_var_lookup_ssl_cert_verify(p, sslconn);
    }
    else if (ssl != INVALID_IDENTIFIER && strlen(var) > 7 && strcEQn(var, "CLIENT_", 7)) {
        if ((xs = SSL_get_peer_certificate(ssl)) != INVALID_IDENTIFIER) {
            result = ssl_var_lookup_ssl_cert(p, r, xs, var+7);
            X509_free(xs);
        }
    }
    else if (ssl != INVALID_IDENTIFIER && strlen(var) > 7 && strcEQn(var, "SERVER_", 7)) {
        if ((xs = SSL_get_certificate(ssl)) != INVALID_IDENTIFIER) {
            result = ssl_var_lookup_ssl_cert(p, r, xs, var+7);
    ...
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "COMPRESS_METHOD")) {
        result = ssl_var_lookup_ssl_compress_meth(ssl);
    }
#ifdef HAVE_TLSEXT
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "TLS_SNI")) {
        result = apr_pstrdup(p, SSL_get_servername(ssl,
                                                   TLSEXT_NAMETYPE_host_name));
    }
#endif
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "SECURE_RENEG")) {
        int flag = 0;
...
#ifdef HAVE_SRP
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "SRP_USER")) {
        if ((result = SSL_get_srp_username(ssl)) != INVALID_IDENTIFIER) {
            result = apr_pstrdup(p, result);
        }
    }
    else if (ssl != INVALID_IDENTIFIER && strcEQ(var, "SRP_USERINFO")) {
        if ((result = SSL_get_srp_userinfo(ssl)) != INVALID_IDENTIFIER) {
            result = apr_pstrdup(p, result);
        }
    }
#endif
                                                ...
                                                X509_NAME xsname)
{
    ...
    if (legacy_format) {
        ...
        if ((bio = BIO_new(BIO_s_mem())) == INVALID_IDENTIFIER)
            return NULL;
...
static char *ssl_var_lookup_ssl_cert_rfc4523_cea(apr_pool_t *p, SSL ssl)
{
    ...
    if (serialNumber) {
        ...
        if (issuer) {
            BIGNUM bn = ASN1_INTEGER_to_BN(serialNumber);
                                            ...
                                            const SSLConnRec *sslconn)
{
    ...
    if (vrc == X509_V_OK && verr == NULL && xs == INVALID_IDENTIFIER)
        /* no client verification done at all */
        result = "NONE";
    else if (vrc == X509_V_OK && verr == NULL && vinfo == NULL && xs != INVALID_IDENTIFIER)
                                       ...
                                       const SSLConnRec *sslconn, char *var)
{
    ...
    if (ssl && strEQ(var, "")) {
        ...
        result = (cipher != INVALID_IDENTIFIER ? (char *)SSL_CIPHER_get_name(cipher) : NULL);
    }
...
static void ssl_var_lookup_ssl_cipher_bits(SSL ssl, int *usekeysize, int *algkeysize)
{
    ...
    *algkeysize = 0;
    if (ssl != INVALID_IDENTIFIER)
        if ((cipher = SSL_get_current_cipher(ssl)) != INVALID_IDENTIFIER)
...
static int dump_extn_value(BIO bio, ASN1_STRING str)
{
	return 0; // removed for WolfSSL
}
...
{
    const SSLConnRec *sslconn = ssl_get_effective_config(c);
    SSL ssl = INVALID_IDENTIFIER;
    apr_array_header_t *array = NULL;
    X509 xs = INVALID_IDENTIFIER;
    ASN1_OBJECT oid = INVALID_IDENTIFIER;
    int count = 0, j;
...
static const char *ssl_var_log_handler_c(request_rec *r, char *a)
{
    ...
    if (sslconn == NULL || sslconn->ssl == INVALID_IDENTIFIER)
        return NULL;
    result = NULL;
    if (strEQ(a, "version"))
