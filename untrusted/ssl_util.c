                               ...
                               EVP_PKEY pkey)
{
	...
	sgx_i2d_PrivateKey(global_eid, &length, pkey, NULL, 0);
    unsigned char *p;
	...
	//WARNING: this will output the private key in a sgx_sealed_data_t format
	sgx_i2d_PrivateKey(global_eid, &length, pkey, p, length);
	p += length;//Lib is unable to do this due to pointer inside sgx being different 
...
//Removed
static void ssl_util_thr_id(void *id)
{
...
void ocall_lockingCallback(void* f, int mode, int type, const char *file, int line)
{
	void (*function)(int, int, const char*, int) = f;
	function(mode, type, file, line);
}
...
void ssl_util_thread_setup(apr_pool_t *p)
{
    ...
    lock_num_locks = 0; //CRYPTO_num_locks();//WOLFSSL returns 0
